...
<header
	class="flex h-14 items-center justify-between border-b border-zinc-800 bg-black px-3 lg:px-4"
>
	<div class="flex min-w-0 items-center gap-3">
		...
		<nav
			class="flex min-w-0 items-center gap-1.5 text-[10px] font-medium tracking-normal text-zinc-500 lowercase"
	...
	<div class="flex items-center gap-2">
		{#if isDashboard}
			<p class="hidden text-xs whitespace-nowrap text-zinc-400 xl:block">{todayLabel}</p>
		{/if}
...
<Drawer
	...
	dismissable={true}
	modal={true}
	class="!top-0 !right-0 !bottom-0 !left-auto !h-[100dvh] !max-h-[100dvh] !w-full overflow-hidden !border-l !border-zinc-900 !bg-black !p-0 !text-zinc-100 sm:!w-80"
>
		...
		<div class="flex items-start justify-between gap-3 border-b border-zinc-900 px-4 py-3">
			<div class="flex items-center gap-3">
				...
				<div>
					...
					<p class="text-[10px] tracking-wider text-zinc-500 uppercase">{user.role}</p>
				</div>
			</div>
			<button
				type="button"
