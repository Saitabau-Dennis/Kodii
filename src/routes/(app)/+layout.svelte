...
<div class="flex h-screen overflow-hidden bg-black text-zinc-100">
  ...
  {#if showSpinner}
    <div class="pointer-events-none fixed inset-0 z-50 flex items-center justify-center">
      <div class="h-8 w-8 animate-spin rounded-full border-2 border-zinc-700 border-t-emerald-400"></div>
    </div>
  {/if}
  ...
  <div class="flex min-h-0 min-w-0 flex-1 flex-col">
    <Topbar user={data.user} pathname={pathname} onMenuClick={openMobileDrawer} />

    <main class="no-scrollbar flex-1 overflow-y-auto bg-black p-4 text-sm">
      {@render children()}
    </main>
  </div>
</div>
