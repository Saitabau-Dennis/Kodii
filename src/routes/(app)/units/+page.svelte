	...
	function handleDeleteSubmit() {
		...
		return async ({ result, update }: { result: ActionResult; update: () => Promise<void> }) => {
			...
			if (result.type === 'failure') {
				deleteError =
					(result.data as { message?: string } | undefined)?.message ?? 'Unable to delete unit';
				return;
			}
...
{#if data.units.length === 0}
	...
	<div class="mt-3 flex justify-center gap-3">
		{#if hasFilters}
			<a href={resolve('/units')} class="text-sm text-emerald-400 hover:text-emerald-300"
				>Clear filters</a
			>
		{/if}
		{#if isLandlord && !hasFilters}
			<Button color="primary" size="sm" onclick={() => (addModalOpen = true)}
				>Add your first unit</Button
			>
		{/if}
	</div>
{:else}
...
{#if isLandlord}
	<form
		method="post"
		action="?/markInactive"
		use:enhance={handleMarkInactiveSubmit}
		bind:this={markInactiveForm}
	>
	...
	>
		<div class="space-y-4 px-6 py-6">
			...
			<p class="text-sm text-zinc-400">
				Are you sure you want to delete Unit {selectedUnit?.unitNumber}? This action cannot be
				undone.
			</p>
	...
	>
		<div class="space-y-4 px-6 py-6">
			...
			<p class="text-sm text-zinc-400">
				Are you sure you want to mark Unit {selectedUnit?.unitNumber} as vacant? This will remove the
