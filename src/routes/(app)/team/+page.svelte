<script lang="ts">
	...
	import { Badge, Button, Drawer, Modal, PaginationNav, Table } from 'flowbite-svelte';
	...
	function handleAssignSubmit() {
		...
		return async ({ result, update }: { result: ActionResult; update: () => Promise<void> }) => {
			...
			if (result.type === 'success') {
				...
				toastStore.error(
					(result.data as { message?: string } | undefined)?.message ?? 'Unable to save'
				);
	...
	function handleDeactivateSubmit() {
		...
		return async ({ result, update }: { result: ActionResult; update: () => Promise<void> }) => {
			...
			if (result.type === 'success') {
				...
				toastStore.success(
					(result.data as { message?: string } | undefined)?.message ?? 'Deactivated'
				);
			} else if (result.type === 'failure') {
				toastStore.error(
					(result.data as { message?: string } | undefined)?.message ?? 'Unable to deactivate'
				);
			} else if (result.type === 'error') {
	...
	function handleReactivateSubmit() {
		...
		return async ({ result, update }: { result: ActionResult; update: () => Promise<void> }) => {
			...
			if (result.type === 'success') {
				...
				toastStore.success(
					(result.data as { message?: string } | undefined)?.message ?? 'Reactivated'
				);
			} else if (result.type === 'failure') {
				toastStore.error(
					(result.data as { message?: string } | undefined)?.message ?? 'Unable to reactivate'
				);
			} else if (result.type === 'error') {
	...
	function handleDeleteSubmit() {
		...
		return async ({ result, update }: { result: ActionResult; update: () => Promise<void> }) => {
			...
			if (result.type === 'success') {
				...
				toastStore.error(
					(result.data as { message?: string } | undefined)?.message ?? 'Unable to delete'
				);
			} else if (result.type === 'error') {
...
<PageHeader>
	...
	{/snippet}
	{#snippet right()}
		<Button color="primary" size="sm" onclick={() => (inviteModalOpen = true)}
			>Invite Caretaker</Button
		>
	{/snippet}
</PageHeader>
			...
			{#each data.caretakers as caretaker (caretaker.id)}
				<article class="rounded-lg border border-zinc-800 p-4">
					<div class="flex items-start justify-between gap-3">
						...
						<span
							class={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-semibold tracking-wide uppercase ${getStatusClass(caretaker.inviteStatus)}`}
						>
					...
					<div class="mt-3 grid grid-cols-2 gap-2 text-xs">
						...
						<div class="px-1 py-1.5">
							...
							<p class="mt-0.5 text-sm font-medium text-zinc-200">
								{formatAddedDate(caretaker.createdAt)}
							</p>
						</div>
					</div>
...
>
	<div class="space-y-6 px-6 py-6 sm:px-7 sm:py-7">
		<div class="space-y-2">
			<h3 class="text-xl leading-tight font-semibold text-zinc-100">Confirm reactivation</h3>
			<p class="max-w-md text-sm leading-relaxed text-zinc-400">
				You are about to restore account access for
				<span class="font-semibold text-zinc-200"
					>{selectedCaretaker?.name ?? 'this caretaker'}</span
				>.
			</p>
		</div>
...
<Drawer
	...
	dismissable={false}
	modal={true}
	class="!top-0 !right-0 !bottom-0 !left-auto !h-screen w-full border-l border-zinc-800 bg-zinc-950 text-zinc-100 sm:w-96"
>
			...
			<div class="flex-1 space-y-2 overflow-y-auto pr-1">
				...
				{:else}
					{#each data.properties as property (property.id)}
						<label
							class="flex items-start gap-2 rounded-lg border border-zinc-800 bg-zinc-900/40 p-2"
						>
							<input
								type="checkbox"
								name="propertyIds"
...
>
	<div class="space-y-6 px-6 py-6 sm:px-7 sm:py-7">
		<div class="space-y-2">
			<h3 class="text-xl leading-tight font-semibold text-zinc-100">Confirm deactivation</h3>
			<p class="max-w-md text-sm leading-relaxed text-zinc-400">
...
>
	<div class="space-y-6 px-6 py-6 sm:px-7 sm:py-7">
		<div class="space-y-2">
			<h3 class="text-xl leading-tight font-semibold text-zinc-100">Confirm deletion</h3>
			<p class="max-w-md text-sm leading-relaxed text-zinc-400">
