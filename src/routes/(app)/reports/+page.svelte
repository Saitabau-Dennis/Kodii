<script lang="ts">
  ...
  const fromLabel = $derived(new Date(data.from).toLocaleDateString('en-KE', { dateStyle: 'medium' }))
  ...
  function exportAllSections() {
    ...
    lines.push(
      toCSV(
        ...
        [[csvKES(data.financialSummary.expectedRent), csvKES(data.financialSummary.confirmedRent), csvKES(data.financialSummary.outstanding), percent(data.financialSummary.collectionRate)]],
      ),
    )
    lines.push('')
...
<div class="space-y-0">
  ...
  <div class="mb-0 mt-3 text-sm text-gray-500 dark:text-gray-400">
  ...
  <section class="mt-8">
    <div class="mb-4 flex items-center justify-between border-b border-gray-200 pb-3 dark:border-gray-800">
      <h2 class="text-base font-semibold text-gray-900 dark:text-white">Financial Summary</h2>
    </div>
  ...
  <ReportTable
    ...
    headers={['Property', 'Units', 'Expected', 'Confirmed', 'Outstanding', 'Collection Rate', 'Overdue Count']}
    rows={rentRows}
  ...
  <section class="mt-8">
    <div class="mb-4 flex items-center justify-between border-b border-gray-200 pb-3 dark:border-gray-800">
      <h2 class="text-base font-semibold text-gray-900 dark:text-white">Occupancy Summary</h2>
    </div>
  ...
  <section class="mt-8">
    <div class="mb-4 flex flex-col gap-3 border-b border-gray-200 pb-3 dark:border-gray-800 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <h3 class="text-base font-semibold text-gray-900 dark:text-white">Overdue Tenants</h3>
        <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">Tenants with unpaid rent past due date</p>
      </div>
      <button
        type="button"
        ...
        onclick={() =>
          exportRows(
            ...
            ['Tenant', 'Phone', 'Unit', 'Property', 'Amount Overdue', 'Days Overdue', 'Last Payment'],
            data.overdueTenants.map((row) => [
              row.tenantName,
              row.tenantPhone,
              row.unitNumber,
  ...
  <section class="mt-8">
    <div class="mb-4 flex flex-col gap-3 border-b border-gray-200 pb-3 dark:border-gray-800 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <h3 class="text-base font-semibold text-gray-900 dark:text-white">Pending Verifications</h3>
        <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">Payments submitted but not yet verified</p>
      </div>
      <div class="flex items-center gap-3">
        ...
        <a href={resolve('/payments?verify=1')} class="text-sm text-primary-600 hover:underline">
          Go to Verify →
        </a>
      </div>
    </div>
            ...
            {:else}
              {#each data.pendingPayments.rows as row (row.id)}
                <tr class="border-t border-gray-100 text-sm text-gray-700 dark:border-gray-800 dark:text-gray-300">
                  ...
                  <td class="px-4 py-3 font-medium text-gray-900 dark:text-white">{formatKES(row.amount)}</td>
    ...
    {#if data.pendingPayments.totalCount > data.pendingPayments.rows.length}
      <a href={resolve('/payments?verify=1')} class="text-sm text-emerald-400 hover:text-emerald-300">
        View all pending →
      </a>
    {/if}
  </section>
  ...
  <section class="mt-8">
    <div class="mb-4 flex items-center justify-between border-b border-gray-200 pb-3 dark:border-gray-800">
      <h2 class="text-base font-semibold text-gray-900 dark:text-white">Maintenance Summary</h2>
    </div>
