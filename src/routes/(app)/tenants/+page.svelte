  ...
  function handleRemoveSubmit() {
    ...
    return async ({ result, update }: { result: ActionResult; update: () => Promise<void> }) => {
      ...
      if (result.type === 'failure') {
        removeError = (result.data as { message?: string } | undefined)?.message ?? 'Unable to remove tenant'
        return
      }
...
{#if data.tenants.length === 0}
  ...
  <div class="mt-3 flex justify-center gap-3">
    {#if hasFilters}
      <a href={resolve('/tenants')} class="text-sm text-emerald-400 hover:text-emerald-300">Clear filters</a>
    {/if}
    {#if isLandlord && !hasFilters}
      <Button color="primary" size="sm" onclick={() => (addModalOpen = true)}>Add your first tenant</Button>
    {/if}
  </div>
{:else}
...
{#if isLandlord}
  ...
  >
    ...
    <div class="p-4">
      {#if selectedTenant}
        <MoveOutForm tenant={selectedTenant} onSuccess={onMovedOut} onCancel={() => (moveOutModalOpen = false)} />
      {/if}
    </div>
  </Modal>
    ...
    <div class="space-y-4 px-6 py-6">
      <div class="space-y-2">
        ...
        <p class="text-sm text-zinc-400">
          Are you sure you want to remove <span class="text-zinc-200">{selectedTenant?.fullName}</span>?
